...
public class AppointmentDao {
	...
	public boolean addAppointment(Appointment ap) {
		...
		try {
			...
			String sql = "insert into appointment(user_id, fullname ,gender ,age, appoint_date , email ,phno ,disease ,docotor_id ,address, status) values(?,?,?,?,?,?,?,?,?,?,?)";
	...
	public List<Appointment> getAllAppointmentByDoctorLogin(int doctorId) {
		...
		try {
			...
			String sql = "select * from appointment where docotor_id =? ";
			PreparedStatement ps = conn.prepareStatement(sql);
	...
	public boolean updateCommentStatus(int id, int doctId, String comm) {
		boolean f = false;
		try {
			String sql = "update appointment set status =? where id =? and docotor_id =? ";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, comm);
			ps.setInt(2, id);
			ps.setInt(3, doctId);
